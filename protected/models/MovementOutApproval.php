...
/**
 * This is the model class for table "rims_movement_out_approval".
 *
 * The followings are the available columns in table 'rims_movement_out_approval':
 * @property integer $id
 ...
 */
class MovementOutApproval extends CActiveRecord
{
	/**
	...
	{
		return 'rims_movement_out_approval';
	}
	...
	public function rules()
	{
		...
		return array(
			array('movement_out_id, revision, approval_type, date, supervisor_id', 'required'),
			array('movement_out_id, revision, supervisor_id', 'numerical', 'integerOnly'=>true),
			array('approval_type', 'length', 'max'=>30),
			array('note', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, movement_out_id, revision, approval_type, date, supervisor_id, note', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return MovementOutApproval the static model class
	 */
