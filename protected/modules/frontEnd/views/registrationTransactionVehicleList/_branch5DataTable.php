<div style="text-align: right">
    <?php echo ReportHelper::summaryText($registrationTransactionVehicleList->dataProviderBranch5); ?>
</div>
...
<div class="table-responsive">
    <table class="table table-sm table-bordered table-hover">
        ...
        <tbody>
            <?php foreach ($registrationTransactionVehicleList->dataProviderBranch5->data as $registrationTransaction): ?>
                <tr>
    ...
    <div class="text-end" id="registration-transaction-branch-5-data-pager">
        <?php $this->widget('system.web.widgets.pagers.CLinkPager', array(
            'pages' => $registrationTransactionVehicleList->dataProviderBranch5->pagination,
        )); ?>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        $('#registration-transaction-branch-5-data-pager ul.yiiPager > li > a').on('click', function(e) {
            e.preventDefault();
            $.ajax({
                type: "GET",
                dataType: "HTML",
                url: "<?php echo CController::createUrl('ajaxHtmlUpdateRegistrationTransactionBranch5DataTable'); ?>&page_branch_5=" + $(this).attr('href').match(/[?&]page_branch_5=([0-9]+)/)[1],
                data: $("form").serialize(),
                success: function(data) {
                    $("#registration_transaction_branch_5_data_container").html(data);
                }
            });
        });
    });
