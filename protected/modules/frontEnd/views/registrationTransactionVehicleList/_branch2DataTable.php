<div style="text-align: right">
    <?php echo ReportHelper::summaryText($registrationTransactionVehicleList->dataProviderBranch2); ?>
</div>
...
<div class="table-responsive">
    <table class="table table-sm table-bordered table-hover">
        ...
        <tbody>
            <?php foreach ($registrationTransactionVehicleList->dataProviderBranch2->data as $registrationTransaction): ?>
                <tr>
    ...
    <div class="text-end" id="registration-transaction-branch-2-data-pager">
        <?php $this->widget('system.web.widgets.pagers.CLinkPager', array(
            'pages' => $registrationTransactionVehicleList->dataProviderBranch2->pagination,
        )); ?>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        $('#registration-transaction-branch-2-data-pager ul.yiiPager > li > a').on('click', function(e) {
            e.preventDefault();
            $.ajax({
                type: "GET",
                dataType: "HTML",
                url: "<?php echo CController::createUrl('ajaxHtmlUpdateRegistrationTransactionBranch2DataTable'); ?>&page_branch_2=" + $(this).attr('href').match(/[?&]page_branch_2=([0-9]+)/)[1],
                data: $("form").serialize(),
                success: function(data) {
                    $("#registration_transaction_branch_2_data_container").html(data);
                }
            });
        });
    });
